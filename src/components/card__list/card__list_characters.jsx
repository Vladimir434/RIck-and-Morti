    ...
    const getData = async () => {
      try {
        ...
        setData(resultJson.results);
          setPages(resultJson.info.pages)
  ...
  return (
    <>
      <div className={s.container__nav}>
        ...
        <div className={s.containet__navigation}>
          <Select
            onHendleChange={(value) => handleFilterChange(value, setSpecies)}
            option={[
              { value: "", label: "Раса" },
        ...
        <div className={s.containet__navigation}>
          <Select
            onHendleChange={(value) => handleFilterChange(value, setGender)}
            option={[
              { value: "", label: "Гендер" },
        ...
        <div className={s.containet__navigation}>
          <Select
            onHendleChange={(value) => handleFilterChange(value, setStatus)}
            option={[
              { value: "", label: "Статус" },
      ...
      <div className={s.card__list}>
        {isError ? (
          ...
          data.map((item, index) => (
            <Link to={`/character/${item.id}`} className={s.card__item} key={index}>
      ...
      <button
        className={s.loat__more}
        onClick={handlePageChange}
      >
        {page < pages ? 'loat more...' : 'start over'}
